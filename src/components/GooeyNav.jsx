...
const GooeyNav = ({
  ...
  const makeParticles = (element) => {
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      setTimeout(() => {
        ...
        particle.style.setProperty('--color', `transparent`);
  ...
  const handleClick = (e, index) => {
    ...
    if (filterRef.current) {
      ...
      particles.forEach((p) => filterRef.current && filterRef.current.removeChild(p));
    }
  ...
  const handleKeyDown = (e, index) => {
    if (e.key === 'Enter' || e.key === ' ') {
      ...
      if (liEl) {
        handleClick({ preventDefault: () => {}, currentTarget: liEl.querySelector('a') || liEl }, index);
      }
    }
  };
